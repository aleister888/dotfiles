#!/bin/bash
#bash script for selecting monitor setup via dmenu
XRANDR=$(which xrandr)
...
# Call menu
SEL=$( gen_entries | dmenu -p "Monitor Setup:" | awk '{print $1}'; sleep 5 && feh --randomize --no-fehbg --bg-fill $HOME/Pictures/Wallpapers)
